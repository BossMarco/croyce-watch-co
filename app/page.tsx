...
const brands = ["ROLEX", "Ω OMEGA", "CARTIER", "TUDOR", "TAG HEUER", "BREITLING", "LONGINES", "SEIKO", "ZENITH", "HAMILTON", "BULOVA", "HEUER"];
...
export default function Home() {
  return <main>
    ...
    <section className="brands"><Reveal><div className="section-label"><span>03</span> Across the watch world</div><div className="brands-intro"><div><p className="eyebrow">No short list here</p><h2>Every brand has<br/><em>its own story.</em></h2></div><p>From familiar icons to under-the-radar references, C. Royce works across eras, styles, and makers. Bring the watch you have—or the watch you cannot stop thinking about.</p></div><div className="brand-wordmarks" aria-label="Examples of watch brands C. Royce can help with">{brands.map((brand, index)=><span className={`brand-wordmark brand-${index + 1}`} key={brand}>{brand}</span>)}</div><div className="brand-callouts"><article><span>01</span><h3>Buy</h3><p>Find the watch that fits your eye—not just the name everyone else is chasing.</p></article><article><span>02</span><h3>Sell & trade</h3><p>Bring a single piece or a full collection for a straightforward conversation.</p></article><article><span>03</span><h3>Source</h3><p>Have a brand, reference, or era in mind? Let&apos;s start there.</p></article></div><p className="brand-disclaimer">Brand names shown are examples of watches we may help buy, sell, trade, consign, or source—not a complete inventory list or an affiliation claim.</p></Reveal></section>

    <section className="watch-feature"><Reveal><div className="section-label"><span>04</span> The details matter</div><div className="watch-feature-grid"><div className="hero-visual feature-visual" aria-label="Vintage mechanical wristwatch"><Image src="/croyce-watch-hero.webp" alt="Vintage mechanical wristwatch on walnut and leather" fill sizes="(max-width: 900px) 100vw, 58vw"/><WatchAtmosphere/><div className="orbit-ring" aria-hidden="true"/><div className="floating-note"><small>Specialists in</small><strong>Vintage</strong></div></div><div className="watch-feature-copy"><p className="eyebrow">Character over convention</p><h2>Made to be<br/><em>remembered.</em></h2><p>Patina, proportion, movement, provenance—every detail adds to the story. We help you find the piece that feels unmistakably yours.</p><a className="text-link" href="https://www.instagram.com/c_royce_co" target="_blank" rel="noreferrer">See what just arrived <ArrowIcon /></a></div></div></Reveal></section>
